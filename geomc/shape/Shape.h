 ...
 * @brief Metadata about shape classes.
 */
...
/**
 * @brief Informational class which reveals whether its `Shape` parameter
 * implements the shape concept `Concept`.
 * 
 * @tparam Shape Shape class to query
 * @tparam Concept Shape concept (e.g., Convex, Bounded, SdfEvaluable, etc.)
 */
template <typename Shape, template <typename, index_t, typename> class Concept>
struct implements_shape_concept : public std::integral_constant<
    bool,
    std::is_class<Concept<typename Shape::elem_t, Shape::N, Shape>>::value and 
    std::is_base_of<Concept<typename Shape::elem_t, Shape::N, Shape>, Shape>::value>
{
// doxygen can't show inherited members of classes outside the library;
// just show the user the thing they need to know exists:
...
/**
 * @brief Informational class which describes its template parameter shape.
 */
template <typename Shape>
struct shape_traits {
