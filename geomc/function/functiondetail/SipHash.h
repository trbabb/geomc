...
inline constexpr void u32to8(uint8_t p[4], uint32_t v) {
    p[0] = (uint8_t)(v);
    p[1] = (uint8_t)(v >> 8);
    p[2] = (uint8_t)(v >> 16);
    p[3] = (uint8_t)(v >> 24);
...
inline constexpr void u64to8(uint8_t p[8], uint64_t v) {
    u32to8(p, v);
    u32to8(p + 4, v >> 32);
}

inline constexpr uint64_t u8to64(const uint8_t p[8]) {
    uint64_t v = p[0];
